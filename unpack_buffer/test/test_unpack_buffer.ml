...
let test (type value) (module V : Value with type t = value) values =
  ...
  List.iter chunk_sizes ~f:(fun chunk_size ->
    ...
    try
      ...
      if not (List.equal V.equal values output)
      then failwiths "mismatch" (values, output) [%sexp_of: V.t list * V.t list]
    with
    | exn ->
      failwiths
        "failure"
        ...
        [%sexp_of:
          exn * [ `chunk_size of int ] * [ `input of Bigstring.t ] * V.t list * V.t t])
;;
...
let%test_unit _ =
  ...
  let sexps =
    Sexp.(
      ...
      ; List [ e; a ]
      ; List [ List [] ]
      ; List [ List []; List [ a ]; List [ a; abc ] ]
      ])
  in
  let test sexps = test (module Value) sexps in
  let terminator = Sexp.List [] in
  (* used to ensure unparsing succeeds *)
...
let%test_unit _ =
  ...
  (let (Unpack_one.T { initial_state; unpack = unpack_sexp }) = Unpack_one.sexp in
   match
     unpack_sexp ~state:initial_state ~buf:(Bigstring.of_string ")") ~pos:0 ~len:1
   with
   | `Invalid_data _ -> ()
   | `Ok _ | `Not_enough_data _ -> assert false);
  ...
  (let (Unpack_one.T { initial_state; unpack = unpack_sexp }) = Unpack_one.sexp in
   match
     unpack_sexp
       ~state:initial_state
       ~buf:(Bigstring.of_string ")(foo)(x y")
       ~pos:1
       ~len:9
   with
   | `Ok (Sexp.List [ Sexp.Atom "foo" ], 5) -> ()
   | `Ok result ->
     Error.raise (Error.create "Unexpected result" result [%sexp_of: Sexp.t * int])
   | `Not_enough_data _ | `Invalid_data _ -> assert false);
  (* Partial sexp case, requries two passes to parse the sexp. *)
  let (Unpack_one.T { initial_state; unpack = unpack_sexp }) = Unpack_one.sexp in
  match
    unpack_sexp ~state:initial_state ~buf:(Bigstring.of_string ")(foo)(x y") ~pos:6 ~len:4
  with
  | `Not_enough_data (k, 4) ->
    (match unpack_sexp ~state:k ~buf:(Bigstring.of_string " z)") ~pos:0 ~len:3 with
     | `Ok (Sexp.List [ Sexp.Atom "x"; Sexp.Atom "y"; Sexp.Atom "z" ], 3) -> ()
     | `Ok _ | `Not_enough_data _ | `Invalid_data _ -> assert false)
...
let%test_unit _ =
  ...
  let module Value = struct
    ...
    let pack ts = List.map ts ~f:(fun sexp -> Sexp.to_string sexp ^ "\n") |> String.concat

    let unpack_one =
      let open Unpack_one.Monad_infix in
      Unpack_one.sexp >>= fun sexp -> Unpack_one.newline >>| fun () -> sexp
    ;;
  end
  in
  test (module Value) [];
...
let%test_unit _ =
  (* [unpack_iter] when [f] raises *)
  let t =
    create
      (Unpack_one.create ~initial_state:() ~unpack:(fun ~state:() ~buf:_ ~pos:_ ~len ->
         if len = 0 then assert false else `Ok ((), 1)))
  in
...
let%expect_test "[feed] and [unpack_iter] allocation" =
  let unpack = Example.bin_reader_t |> Unpack_one.create_bin_prot |> create in
