...
module Definitions = struct
  ...
  type ('src, 'dst) consuming_blit =
    src:'src -> dst:'dst -> dst_pos:int -> len:int -> unit
  ...
  module type Consume_safe = sig
    ...
    include
      Accessors_read
      with type ('a, 'r, 's, 'l) t = (([> read ] as 'r), seek, 'l) iobuf -> 'a
      with type ('a, 'r, 's, 'l) t__local = (([> read ] as 'r), seek, 'l) iobuf -> 'a
  end
  ...
  module type Fill_safe = sig
    ...
    include
      Accessors_write
      with type ('a, 'd, 'w, 'l) t = (read_write, seek, 'l) iobuf -> 'a -> unit
      with type ('a, 'd, 'w, 'l) t__local = (read_write, seek, 'l) iobuf -> 'a -> unit
  ...
  module type Peek_common = sig
    ...
    include
      Accessors_read
      with type ('a, 'd, 'w, 'l) t = ('d, 'w, 'l) iobuf -> pos:int -> 'a
      with type ('a, 'd, 'w, 'l) t__local = ('d, 'w, 'l) iobuf -> pos:int -> 'a
  end
  ...
  module type Peek_safe = sig
    ...
    val index : ([> read ], _, _) iobuf -> ?pos:int -> ?len:int -> char -> int option
    ...
    (** [rindex ?pos ?len t c] returns [Some i] for the largest [i >= pos] such that
        ...
        @param pos default = 0
        @param len default = [length t - pos] *)
    val rindex : ([> read ], _, _) iobuf -> ?pos:int -> ?len:int -> char -> int option
  end
  ...
  module type Poke_safe = sig
    ...
    val decimal : (read_write, 'w, 'l) iobuf -> pos:int -> int -> int

    (** Same as [decimal t int], but padding to [len] with prefix '0's. *)
    val padded_decimal : (read_write, 'w, 'l) iobuf -> pos:int -> len:int -> int -> int

    include
      Accessors_write
      with type ('a, 'd, 'w, 'l) t = (read_write, 'w, 'l) iobuf -> pos:int -> 'a -> unit
      with type ('a, 'd, 'w, 'l) t__local =
        (read_write, 'w, 'l) iobuf -> pos:int -> 'a -> unit
