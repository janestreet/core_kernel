...
module type Iobuf_type = sig
  ...
  val bad_range : pos:int -> len:int -> (_, _, _) t -> _
  ...
  val check_range : (_, _, _) t -> pos:int -> len:int -> unit
  val create : len:int -> (_, _, _) t
  val fail : (_, _, _) t -> string -> 'a -> ('a -> Sexp.t) -> _
  val get_char : ([> read ], _, _) t -> int -> char
  val length : (_, _, _) t -> int
  val set_bounds_and_buffer : src:(_, _, global) t -> dst:(_, _, _) t -> unit
  ...
  val set_char : ([> write ], _, _) t -> int -> char -> unit
