...
module Definitions = struct
  ...
  module type Accessors_read = sig
    ...
    module Local : sig
      ...
      val stringo
        :  ?str_pos:local_ int
        -> ?len:local_ int
        -> local_ (string, 'd, 'w) t_local

      val byteso
        :  ?str_pos:local_ int
        -> ?len:local_ int
        -> local_ (Bytes.t, 'd, 'w) t_local
    end
  ...
  module type Accessors_write = sig
    ...
    val bigstringo
      :  ?str_pos:local_ int
      -> ?len:local_ int
      -> local_ (Bigstring.t, 'd, 'w) t_local
  ...
  module type Peek = sig
    ...
    include
      ...
      with type ('a, 'd, 'w) t_local = local_ ('d, 'w) iobuf -> pos:int -> local_ 'a
...
module type Iobuf = sig
  ...
  val globalize : _ -> _ -> local_ ('rw, _) t -> ('rw, _) t
  ...
  type ('rw, 'seek) t_with_shallow_sexp = ('rw, 'seek) t [@@deriving sexp_of, globalize]
  ...
  val bigstring_view : pos:int -> len:int -> Bigstring.t -> local_ ([< read_write ], _) t

  (** [of_string s] returns a new iobuf whose contents are [s]. *)
  val of_string : local_ string -> (_, _) t

  (** [of_string_local] is like [of_string], but it allocates the iobuf record locally.
      This still performs global allocation of the backing buffer. *)
  val of_string_local : local_ string -> local_ (_, _) t
  ...
  val read_only_local : local_ ([> read ], 's) t -> local_ (read, 's) t
  val no_seek : ('r, _) t -> ('r, no_seek) t
  val no_seek_local : local_ ('r, _) t -> local_ ('r, no_seek) t
  ...
  module Consume : sig
    ...
    include
      Accessors_read
      with type ('a, 'r, 's) t = local_ (([> read ] as 'r), seek) t -> 'a
      with type ('a, 'r, 's) t_local = local_ (([> read ] as 'r), seek) t -> local_ 'a
      with type 'a bin_prot := 'a Bin_prot.Type_class.reader
  end
  ...
  module Fill : sig
    include
      Accessors_write
      with type ('a, 'd, 'w) t = local_ (read_write, seek) t -> 'a -> unit
      with type ('a, 'd, 'w) t_local = local_ (read_write, seek) t -> local_ 'a -> unit
  ...
  module Poke : sig
    ...
    val decimal : local_ (read_write, 'w) t -> pos:int -> int -> int

    (** Same as [decimal t int], but padding to [len] with prefix '0's. *)
    val padded_decimal : local_ (read_write, 'w) t -> pos:int -> len:int -> int -> int
    ...
    include
      Accessors_write
      with type ('a, 'd, 'w) t = local_ (read_write, 'w) t -> pos:int -> 'a -> unit
  ...
  module Expert : sig
    ...
    val hi_max : local_ (_, _) t -> int
    val hi : local_ (_, _) t -> int
    val lo : local_ (_, _) t -> int
    val lo_min : local_ (_, _) t -> int

    (** These setters directly set fields in [t] without checking any invariants. *)
    val set_buf : local_ (_, _) t -> Bigstring.t -> unit

    val set_hi_max : local_ (_, _) t -> int -> unit
    val set_hi : local_ (_, _) t -> int -> unit
    val set_lo : local_ (_, _) t -> int -> unit
    val set_lo_min : local_ (_, _) t -> int -> unit
    ...
    val to_bigstring_shared : ?pos:int -> ?len:int -> local_ (_, _) t -> Bigstring.t
    ...
    val protect_window : local_ ('rw, _) t -> f:local_ (local_ ('rw, seek) t -> 'a) -> 'a
    ...
    val protect_window_2
      ...
      -> 'a
      -> 'b
      -> f:local_ (local_ ('rw, seek) t -> 'a -> 'b -> 'c)
      -> 'c
    ...
    val protect_window_local
      :  local_ ('rw, _) t
      -> f:local_ (local_ ('rw, seek) t -> local_ 'a)
      -> local_ 'a
