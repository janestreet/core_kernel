...
module Definitions = struct
  module type Bin_io = sig
    ...
    val consume : 'a Bin_prot.Type_class.reader -> ([> read ], seek) t @ local -> 'a
    ...
    val fill : 'a Bin_prot.Type_class.writer -> (read_write, seek) t @ local -> 'a -> unit

    val fill__local
      :  'a Bin_prot.Size.sizer__local
      -> 'a Bin_prot.Write.writer__local
      -> (read_write, seek) t @ local
      -> 'a @ local
      -> unit
