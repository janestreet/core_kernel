...
module Definitions = struct
  ...
  module type Basic = sig
    ...
    val globalize_shared : ('rw, _, global) t -> ('rw, _, global) t
    val globalize_copied : ('rw, _, 'loc) t -> ('rw, _, 'loc) t
    ...
    val of_bigstring
      ...
      -> Bigstring.t
      -> ([< read_write ], _, (_ locality[@mode l])) t
    ...
    val%template of_string : string -> (_, _, _) t
    ...
    val sub_shared : ?pos:int -> ?len:int -> ('d, _, global) t -> ('d, _, global) t

    (** [sub_shared__local] is like [sub_shared], but it allocates the iobuf record
        locally. *)
    val sub_shared__local : ?pos:int -> ?len:int -> ('d, _, 'loc) t -> ('d, _, 'loc) t

    (** More efficient than the above (optional arguments are costly). *)
    val unsafe_sub_shared : pos:int -> len:int -> ('d, _, 'loc) t -> ('d, _, 'loc) t

    (** [copy t] returns a new iobuf whose contents are the same as those in the window of
        [t]. *)
    val copy : (_, _, _) t -> (_, _, _) t
    ...
    val transfer : src:([> read ], _, _) t -> dst:([> write ], seek, _) t -> unit
    ...
    val flip_lo : (_, seek, _) t -> unit

    val bounded_flip_lo : (_, seek, _) t -> Lo_bound.t -> unit
    ...
    val compact : (read_write, seek, _) t -> unit

    val bounded_compact : (read_write, seek, _) t -> Lo_bound.t -> Hi_bound.t -> unit
    ...
    val flip_hi : (_, seek, _) t -> unit

    val bounded_flip_hi : (_, seek, _) t -> Hi_bound.t -> unit
    ...
    val to_bytes : ?len:int -> (_, _, _) t -> Bytes.t
