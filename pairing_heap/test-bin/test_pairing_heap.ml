...
let to_sorted_list h = List.rev (List.init ~f:(fun _ -> Heap.pop_exn h) (Heap.length h))
...
let test =
  ...
  in
  [ ("length" >:: fun () -> "length=4" @? (Heap.length (int_heap ()) = 4))
  ; ("is_empty"
     >:: fun () ->
  ...
  ; ("pop_if"
     >:: fun () ->
       ...
       ("dopop"
        @?
        match Heap.pop_if h (fun i -> i < 0) with
        | None -> false
        | Some t -> t = -1);
       ...
       "empty"
       @?
       match Heap.pop_if empty (fun _ -> true) with
       | None -> true
       | Some _ -> false)
  ; ("search functions"
     >:: fun () ->
       ...
       "find"
       @?
       let el = Option.value_exn (Heap.find (int_heap ()) ~f:(fun e -> e = 2)) in
       2 = el)
  ; ("iter"
     >:: fun () ->
       "content differs"
  ...
  ; ("random heap"
     >:: fun () ->
       ...
       with
       | _ -> if Heap.is_empty random_heap then true else false)
  ; ("sort"
     >:: fun () ->
       ("randomints"
        @?
