...
(** If [t] and [token] are mismatched then behavior is undefined. Trying to [remove] an
    already removed token (by an earlier call to [remove] or [pop] for instance) is a
    no-op, but keeping [token] around after it has been removed may lead to memory leaks
    since it has a reference to the heap. *)
...
module Unsafe : sig
  ...
  (** [add_removable t v] returns a token that can be later used to remove [v]. Unlike the
      regular function, this unsafe version doesn't allocate. *)
  val add_removable : 'a t -> 'a -> 'a Elt.t
