          ...
          ~f:(fun acc k ->
            Int64.( lor ) acc (
              ...
              if signed && num_bytes < 8 then
                ...
                if w >= max_val then
                  Int64.( - ) w (Int64.shift_left max_val 1)
                else
                  w
              else
                w))))
...
let%test_module "inline_unsigned_32_int" [@tags "64-bits-only"] = (module Make_inline_tests (struct
    let ns = [0x3f20_3040; Int64.to_int_exn 0x7f20_3040L;
...
let%test_module "inline_signed_32_int" [@tags "64-bits-only"] = (module Make_inline_tests (struct
    let ns = [0x3f20_3040; Int64.to_int_exn 0x7f20_3040L; Int64.to_int_exn (-0x7f20_3040L);
  ...
  List.iter
    ...
    ~f:(fun (pack, unpack) ->
      List.iter [
        ...
        with Private.Unpack_signed_64_int_most_significant_byte_too_large n when Some n = opt -> ()
      ))
;;
