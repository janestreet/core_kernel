...
let%expect_test ("Byte_units.to_string_hum"[@tags "64-bits-only"]) =
  ...
  [%expect {| 1.46484k |}];
  print_string
    (Byte_units.to_string_hum ~measure:`Gigabytes (Byte_units.create `Bytes 1000.));
  [%expect {| 9.31323e-07g |}];
  print_string
    (Byte_units.to_string_hum ~measure:`Words (Byte_units.create `Bytes 1000.));
  [%expect {| 125w |}]
;;
...
let%test_module "{of,to}_string" =
  (module struct
    let f measure input expected_output =
      let observed_output =
        match measure with
        | `Specific measure -> to_string_hum ~measure (of_string input)
        | `Largest -> to_string_hum (of_string input)
      in
      let result = String.equal expected_output observed_output in
      if not result
      then (
        let measure =
          [%sexp_of: [`Specific of Measure.t | `Largest]] measure |> Sexp.to_string
        in
    ...
    let%test _ =
      f (`Specific `Words) "3k" (sprintf "%gw" (3.0 *. kbyte /. bytes_per_word))
    ;;
  end)
;;
