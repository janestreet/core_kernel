...
module Make (Timing_wheel : Timing_wheel)
  ...
  let%test_unit _ =
    List.iter
      ...
      ~f:(fun reschedule ->
        ...
        [%test_eq: Time.t] (max_alarm_time_in_min_interval_exn t) (epoch_plus 5.);
        [%test_eq: Interval_num.t] (min_alarm_interval_num_exn t) (Interval_num.of_int 5);
  ...
  let max_alarm_time_in_min_interval = max_alarm_time_in_min_interval

  let max_alarm_time_in_min_interval_exn = max_alarm_time_in_min_interval_exn

  let min_alarm_interval_num = min_alarm_interval_num

  let min_alarm_interval_num_exn = min_alarm_interval_num_exn
  ...
  let%test_unit _ =
    ...
    let no_next_alarm () =
      is_none (next_alarm_fires_at t) && is_none (max_alarm_time_in_min_interval t)
    in
