...
module Span = struct
  ...
  let%test _ [@tags "64-bits-only"] =
    Int.(>) (to_int_sec (of_int63_ns Int63.max_value)) 0 (* and doesn't raise *)
      ...
      let%test_module "intermediate ( * )" =
        (module struct
          let wrap_days =
            ...
            Int63.(max_value / to_int63_ns day + one
                   + margin_ns / to_int63_ns day + one)
          ;;
  ...
  let%test_module _ = (module struct
    ...
    let%expect_test _ =
      print_s
        [%sexp (to_parts (create ~sign:Neg ~hr:2 ~min:3 ~sec:4 ~ms:5 ~us:6 ~ns:7 ())
                : Parts.t)];
      [%expect {|
        ((sign Neg)
         (hr   2)
