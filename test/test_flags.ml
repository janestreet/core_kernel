open! Core_kernel.Std
open! Expect_test_helpers_kernel.Std
open! Flags

let a = Int63.of_int 0x1
let b = Int63.of_int 0x2
let c = Int63.of_int 0xC
let d = create ~bit:62

let%expect_test _ =
  List.iter [ -1; 63 ] ~f:(fun bit ->
    show_raise ~hide_positions:true (fun () -> ignore (create ~bit : Int63.t)));
  [%expect {|
    (raised (
      exn (
        "Flags.create got invalid ~bit (must be between 0 and 62)"
        -1
        lib/core_kernel/src/flags.ml:LINE:COL)))
    (raised (
      exn (
        "Flags.create got invalid ~bit (must be between 0 and 62)"
        63
        lib/core_kernel/src/flags.ml:LINE:COL))) |}];
;;

let%expect_test _ =
  let print_hex bit = print_s [%sexp (create ~bit |> Int63.Hex.to_string_hum : string)] in
  print_hex 0;
  [%expect {|
    0x1 |}];
  print_hex 1;
  [%expect {|
    0x2 |}];
  print_hex 62;
  [%expect {|
    -0x4000_0000_0000_0000 |}];
;;

module M = Make (struct
    let allow_intersecting = false
    let should_print_error = true
    let known =
      [ a, "a";
        b, "b";
        c, "c";
      ]
    ;;
    let remove_zero_flags = false
  end)

include M

include Comparable.Check_sexp_conversion (struct
    include M
    let examples = [ a; b; c ]
  end)

(* [sexp_of_t] *)
let print_sexp_of t = print_s [%sexp (t : t)]

let%expect_test _ =
  print_sexp_of empty;
  [%expect {|
    () |}];
;;

let%expect_test _ =
  print_sexp_of a;
  [%expect {|
    (a) |}];
;;

let%expect_test _ =
  print_sexp_of c;
  [%expect {|
    (c) |}];
;;

let%expect_test _ =
  print_sexp_of (a + b);
  [%expect {|
    (a b) |}];
;;

let%expect_test _ =
  print_sexp_of (Int63.of_int 0x10);
  [%expect {|
    (() (unrecognized_bits 0x10)) |}];
;;

(* [t_of_sexp] *)
let check_t_of_sexp here expect string =
  require here (equal expect (t_of_sexp (Sexp.of_string string)))
;;

let%expect_test _ = check_t_of_sexp [%here] empty "()"
let%expect_test _ = check_t_of_sexp [%here] a "(a)"
let%expect_test _ = check_t_of_sexp [%here] c "(c)"
let%expect_test _ = check_t_of_sexp [%here] (b + c) "(b c)"
let%expect_test _ = check_t_of_sexp [%here] (b + c) "(c b)"

let%expect_test _ =
  List.iter [ "a"; "(())"; "(a ())"; "(d)" ] ~f:(fun string ->
    show_raise (fun () ->
      print_s [%message "" ~input:string];
      ignore (t_of_sexp (Sexp.of_string string) : t)));
  [%expect {|
    (input a)
    (raised (
      exn (Sexplib.Conv.Of_sexp_error (Failure "list_of_sexp: list needed") a)))
    (input "(())")
    (raised (
      exn (Sexplib.Conv.Of_sexp_error (Failure "string_of_sexp: atom needed") ())))
    (input "(a ())")
    (raised (
      exn (Sexplib.Conv.Of_sexp_error (Failure "string_of_sexp: atom needed") ())))
    (input "(d)")
    (raised (
      exn (
        Sexplib.Conv.Of_sexp_error
        (Failure "Flags.t_of_sexp got unknown name: d")
        (d)))) |}];
;;

(* +, - *)
let%expect_test _ = require [%here] (equal (a + a) a)
let%expect_test _ = require [%here] (equal (a + b) (b + a))
let%expect_test _ = require [%here] (equal (a - a) empty)
let%expect_test _ = require [%here] (equal ((a + b) - a) b)

(* [intersect] *)
let%expect_test _ = require [%here] (equal (intersect a a) a)
let%expect_test _ = require [%here] (equal (intersect a b) empty)
let%expect_test _ = require [%here] (equal (intersect (a + b) a) a)

(* [complement] *)
let%expect_test _ = require [%here] (equal (intersect (complement a) b) b)

(* [do_intersect] *)
let%expect_test _ = require [%here] (do_intersect a a)
let%expect_test _ = require [%here] (not (do_intersect a b))
let%expect_test _ = require [%here] (do_intersect (a + b) a)
let%expect_test _ = require [%here] (do_intersect (a + b) b)
let%expect_test _ = require [%here] (not (do_intersect (a + b) c))

(* [are_disjoint] *)
let%expect_test _ = require [%here] (are_disjoint a empty)
let%expect_test _ = require [%here] (not (are_disjoint a a))
let%expect_test _ = require [%here] (are_disjoint a b)
let%expect_test _ = require [%here] (are_disjoint b a)
let%expect_test _ = require [%here] (not (are_disjoint (a + b) a))
let%expect_test _ = require [%here] (are_disjoint (a + b) c)

(* compare *)
let%expect_test _ = require [%here] (Int.(=) (Int.compare 0 1) (-1))

let print_compare t1 t2 = print_s [%sexp (compare t1 t2 : int)]

let%expect_test _ =
  print_compare a empty;
  [%expect {|
    1 |}];
  print_compare c empty;
  [%expect {|
    1 |}];
  print_compare a a;
  [%expect {|
    0 |}];
  print_compare c c;
  [%expect {|
    0 |}];
  print_compare empty empty;
  [%expect {|
    0 |}];
  print_compare empty a;
  [%expect {|
    -1 |}];
  print_compare empty c;
  [%expect {|
    -1 |}];
  print_compare (a + c) a;
  [%expect {|
    1 |}];
  print_compare (a + c) c;
  [%expect {|
    1 |}];
  print_compare (b + b) b;
  [%expect {|
    0 |}];
  print_compare b (b + c);
  [%expect {|
    -1 |}];
;;

let%expect_test "transitivity of [compare]" =
  print_compare a b;
  [%expect {|
    -1 |}];
  print_compare b (b + d);
  [%expect {|
    -1 |}];
  print_compare a (b + d);
  [%expect {|
    1 |}];
  let ordered =
    [| empty
     ; a
     ; b
     ; a+b
     ; c
     ; a+c
     ; b+c
     ; a+b+c
     ; complement d
     ; d
     ; a+d
     ; b+d
     ; complement (a + b)
     ; complement b
     ; complement a
     ; complement empty |]
  in
  Array.iteri ordered ~f:(fun i ai ->
    Array.iteri ordered ~f:(fun j aj ->
      if not (Int.equal (compare ai aj) (Int.compare i j))
      then print_s [%message
             "bug in Flags.compare"
               (i : int) (j : int) (ai : t) (aj : t) (compare ai aj : int)]));
  [%expect {|
    ("bug in Flags.compare"
      (i 1)
      (j 9)
      (ai (a))
      (aj (() (unrecognized_bits 0xc000000000000000)))
      ("compare ai aj" 1))
    ("bug in Flags.compare"
      (i 1)
      (j 11)
      (ai (a))
      (aj ((b) (unrecognized_bits 0xc000000000000000)))
      ("compare ai aj" 1))
    ("bug in Flags.compare"
      (i 1)
      (j 12)
      (ai (a))
      (aj ((c) (unrecognized_bits 0xfffffffffffffff0)))
      ("compare ai aj" 1))
    ("bug in Flags.compare"
      (i 1)
      (j 14)
      (ai (a))
      (aj (
        (b                 c)
        (unrecognized_bits 0xfffffffffffffff0)))
      ("compare ai aj" 1))
    ("bug in Flags.compare"
      (i 2)
      (j 9)
      (ai (b))
      (aj (() (unrecognized_bits 0xc000000000000000)))
      ("compare ai aj" 1))
    ("bug in Flags.compare"
      (i 2)
      (j 10)
      (ai (b))
      (aj ((a) (unrecognized_bits 0xc000000000000000)))
      ("compare ai aj" 1))
    ("bug in Flags.compare"
      (i 2)
      (j 12)
      (ai (b))
      (aj ((c) (unrecognized_bits 0xfffffffffffffff0)))
      ("compare ai aj" 1))
    ("bug in Flags.compare"
      (i 2)
      (j 13)
      (ai (b))
      (aj (
        (a                 c)
        (unrecognized_bits 0xfffffffffffffff0)))
      ("compare ai aj" 1))
    ("bug in Flags.compare"
      (i 3)
      (j 9)
      (ai (a b))
      (aj (() (unrecognized_bits 0xc000000000000000)))
      ("compare ai aj" 1))
    ("bug in Flags.compare"
      (i 3)
      (j 10)
      (ai (a b))
      (aj ((a) (unrecognized_bits 0xc000000000000000)))
      ("compare ai aj" 1))
    ("bug in Flags.compare"
      (i 3)
      (j 11)
      (ai (a b))
      (aj ((b) (unrecognized_bits 0xc000000000000000)))
      ("compare ai aj" 1))
    ("bug in Flags.compare"
      (i 3)
      (j 12)
      (ai (a b))
      (aj ((c) (unrecognized_bits 0xfffffffffffffff0)))
      ("compare ai aj" 1))
    ("bug in Flags.compare"
      (i 3)
      (j 13)
      (ai (a b))
      (aj (
        (a                 c)
        (unrecognized_bits 0xfffffffffffffff0)))
      ("compare ai aj" 1))
    ("bug in Flags.compare"
      (i 3)
      (j 14)
      (ai (a b))
      (aj (
        (b                 c)
        (unrecognized_bits 0xfffffffffffffff0)))
      ("compare ai aj" 1))
    ("bug in Flags.compare"
      (i 4)
      (j 9)
      (ai (c))
      (aj (() (unrecognized_bits 0xc000000000000000)))
      ("compare ai aj" 1))
    ("bug in Flags.compare"
      (i 4)
      (j 10)
      (ai (c))
      (aj ((a) (unrecognized_bits 0xc000000000000000)))
      ("compare ai aj" 1))
    ("bug in Flags.compare"
      (i 4)
      (j 11)
      (ai (c))
      (aj ((b) (unrecognized_bits 0xc000000000000000)))
      ("compare ai aj" 1))
    ("bug in Flags.compare"
      (i 5)
      (j 9)
      (ai (a c))
      (aj (() (unrecognized_bits 0xc000000000000000)))
      ("compare ai aj" 1))
    ("bug in Flags.compare"
      (i 5)
      (j 10)
      (ai (a c))
      (aj ((a) (unrecognized_bits 0xc000000000000000)))
      ("compare ai aj" 1))
    ("bug in Flags.compare"
      (i 5)
      (j 11)
      (ai (a c))
      (aj ((b) (unrecognized_bits 0xc000000000000000)))
      ("compare ai aj" 1))
    ("bug in Flags.compare"
      (i 5)
      (j 12)
      (ai (a c))
      (aj ((c) (unrecognized_bits 0xfffffffffffffff0)))
      ("compare ai aj" 1))
    ("bug in Flags.compare"
      (i 5)
      (j 14)
      (ai (a c))
      (aj (
        (b                 c)
        (unrecognized_bits 0xfffffffffffffff0)))
      ("compare ai aj" 1))
    ("bug in Flags.compare"
      (i 6)
      (j 9)
      (ai (b c))
      (aj (() (unrecognized_bits 0xc000000000000000)))
      ("compare ai aj" 1))
    ("bug in Flags.compare"
      (i 6)
      (j 10)
      (ai (b c))
      (aj ((a) (unrecognized_bits 0xc000000000000000)))
      ("compare ai aj" 1))
    ("bug in Flags.compare"
      (i 6)
      (j 11)
      (ai (b c))
      (aj ((b) (unrecognized_bits 0xc000000000000000)))
      ("compare ai aj" 1))
    ("bug in Flags.compare"
      (i 6)
      (j 12)
      (ai (b c))
      (aj ((c) (unrecognized_bits 0xfffffffffffffff0)))
      ("compare ai aj" 1))
    ("bug in Flags.compare"
      (i 6)
      (j 13)
      (ai (b c))
      (aj (
        (a                 c)
        (unrecognized_bits 0xfffffffffffffff0)))
      ("compare ai aj" 1))
    ("bug in Flags.compare"
      (i 7)
      (j 9)
      (ai (a b c))
      (aj (() (unrecognized_bits 0xc000000000000000)))
      ("compare ai aj" 1))
    ("bug in Flags.compare"
      (i 7)
      (j 10)
      (ai (a b c))
      (aj ((a) (unrecognized_bits 0xc000000000000000)))
      ("compare ai aj" 1))
    ("bug in Flags.compare"
      (i 7)
      (j 11)
      (ai (a b c))
      (aj ((b) (unrecognized_bits 0xc000000000000000)))
      ("compare ai aj" 1))
    ("bug in Flags.compare"
      (i 7)
      (j 12)
      (ai (a b c))
      (aj ((c) (unrecognized_bits 0xfffffffffffffff0)))
      ("compare ai aj" 1))
    ("bug in Flags.compare"
      (i 7)
      (j 13)
      (ai (a b c))
      (aj (
        (a                 c)
        (unrecognized_bits 0xfffffffffffffff0)))
      ("compare ai aj" 1))
    ("bug in Flags.compare"
      (i 7)
      (j 14)
      (ai (a b c))
      (aj (
        (b                 c)
        (unrecognized_bits 0xfffffffffffffff0)))
      ("compare ai aj" 1))
    ("bug in Flags.compare"
      (i 8)
      (j 9)
      (ai ((a b c) (unrecognized_bits 0x3ffffffffffffff0)))
      (aj (() (unrecognized_bits 0xc000000000000000)))
      ("compare ai aj" 1))
    ("bug in Flags.compare"
      (i 8)
      (j 10)
      (ai ((a b c) (unrecognized_bits 0x3ffffffffffffff0)))
      (aj ((a) (unrecognized_bits 0xc000000000000000)))
      ("compare ai aj" 1))
    ("bug in Flags.compare"
      (i 8)
      (j 11)
      (ai ((a b c) (unrecognized_bits 0x3ffffffffffffff0)))
      (aj ((b) (unrecognized_bits 0xc000000000000000)))
      ("compare ai aj" 1))
    ("bug in Flags.compare"
      (i 8)
      (j 12)
      (ai ((a b c) (unrecognized_bits 0x3ffffffffffffff0)))
      (aj ((c) (unrecognized_bits 0xfffffffffffffff0)))
      ("compare ai aj" 1))
    ("bug in Flags.compare"
      (i 8)
      (j 13)
      (ai ((a b c) (unrecognized_bits 0x3ffffffffffffff0)))
      (aj (
        (a                 c)
        (unrecognized_bits 0xfffffffffffffff0)))
      ("compare ai aj" 1))
    ("bug in Flags.compare"
      (i 8)
      (j 14)
      (ai ((a b c) (unrecognized_bits 0x3ffffffffffffff0)))
      (aj (
        (b                 c)
        (unrecognized_bits 0xfffffffffffffff0)))
      ("compare ai aj" 1))
    ("bug in Flags.compare"
      (i 9)
      (j 1)
      (ai (() (unrecognized_bits 0xc000000000000000)))
      (aj (a))
      ("compare ai aj" -1))
    ("bug in Flags.compare"
      (i 9)
      (j 2)
      (ai (() (unrecognized_bits 0xc000000000000000)))
      (aj (b))
      ("compare ai aj" -1))
    ("bug in Flags.compare"
      (i 9)
      (j 3)
      (ai (() (unrecognized_bits 0xc000000000000000)))
      (aj (a b))
      ("compare ai aj" -1))
    ("bug in Flags.compare"
      (i 9)
      (j 4)
      (ai (() (unrecognized_bits 0xc000000000000000)))
      (aj (c))
      ("compare ai aj" -1))
    ("bug in Flags.compare"
      (i 9)
      (j 5)
      (ai (() (unrecognized_bits 0xc000000000000000)))
      (aj (a c))
      ("compare ai aj" -1))
    ("bug in Flags.compare"
      (i 9)
      (j 6)
      (ai (() (unrecognized_bits 0xc000000000000000)))
      (aj (b c))
      ("compare ai aj" -1))
    ("bug in Flags.compare"
      (i 9)
      (j 7)
      (ai (() (unrecognized_bits 0xc000000000000000)))
      (aj (a b c))
      ("compare ai aj" -1))
    ("bug in Flags.compare"
      (i 9)
      (j 8)
      (ai (() (unrecognized_bits 0xc000000000000000)))
      (aj ((a b c) (unrecognized_bits 0x3ffffffffffffff0)))
      ("compare ai aj" -1))
    ("bug in Flags.compare"
      (i 10)
      (j 2)
      (ai ((a) (unrecognized_bits 0xc000000000000000)))
      (aj (b))
      ("compare ai aj" -1))
    ("bug in Flags.compare"
      (i 10)
      (j 3)
      (ai ((a) (unrecognized_bits 0xc000000000000000)))
      (aj (a b))
      ("compare ai aj" -1))
    ("bug in Flags.compare"
      (i 10)
      (j 4)
      (ai ((a) (unrecognized_bits 0xc000000000000000)))
      (aj (c))
      ("compare ai aj" -1))
    ("bug in Flags.compare"
      (i 10)
      (j 5)
      (ai ((a) (unrecognized_bits 0xc000000000000000)))
      (aj (a c))
      ("compare ai aj" -1))
    ("bug in Flags.compare"
      (i 10)
      (j 6)
      (ai ((a) (unrecognized_bits 0xc000000000000000)))
      (aj (b c))
      ("compare ai aj" -1))
    ("bug in Flags.compare"
      (i 10)
      (j 7)
      (ai ((a) (unrecognized_bits 0xc000000000000000)))
      (aj (a b c))
      ("compare ai aj" -1))
    ("bug in Flags.compare"
      (i 10)
      (j 8)
      (ai ((a) (unrecognized_bits 0xc000000000000000)))
      (aj ((a b c) (unrecognized_bits 0x3ffffffffffffff0)))
      ("compare ai aj" -1))
    ("bug in Flags.compare"
      (i 11)
      (j 1)
      (ai ((b) (unrecognized_bits 0xc000000000000000)))
      (aj (a))
      ("compare ai aj" -1))
    ("bug in Flags.compare"
      (i 11)
      (j 3)
      (ai ((b) (unrecognized_bits 0xc000000000000000)))
      (aj (a b))
      ("compare ai aj" -1))
    ("bug in Flags.compare"
      (i 11)
      (j 4)
      (ai ((b) (unrecognized_bits 0xc000000000000000)))
      (aj (c))
      ("compare ai aj" -1))
    ("bug in Flags.compare"
      (i 11)
      (j 5)
      (ai ((b) (unrecognized_bits 0xc000000000000000)))
      (aj (a c))
      ("compare ai aj" -1))
    ("bug in Flags.compare"
      (i 11)
      (j 6)
      (ai ((b) (unrecognized_bits 0xc000000000000000)))
      (aj (b c))
      ("compare ai aj" -1))
    ("bug in Flags.compare"
      (i 11)
      (j 7)
      (ai ((b) (unrecognized_bits 0xc000000000000000)))
      (aj (a b c))
      ("compare ai aj" -1))
    ("bug in Flags.compare"
      (i 11)
      (j 8)
      (ai ((b) (unrecognized_bits 0xc000000000000000)))
      (aj ((a b c) (unrecognized_bits 0x3ffffffffffffff0)))
      ("compare ai aj" -1))
    ("bug in Flags.compare"
      (i 12)
      (j 1)
      (ai ((c) (unrecognized_bits 0xfffffffffffffff0)))
      (aj (a))
      ("compare ai aj" -1))
    ("bug in Flags.compare"
      (i 12)
      (j 2)
      (ai ((c) (unrecognized_bits 0xfffffffffffffff0)))
      (aj (b))
      ("compare ai aj" -1))
    ("bug in Flags.compare"
      (i 12)
      (j 3)
      (ai ((c) (unrecognized_bits 0xfffffffffffffff0)))
      (aj (a b))
      ("compare ai aj" -1))
    ("bug in Flags.compare"
      (i 12)
      (j 5)
      (ai ((c) (unrecognized_bits 0xfffffffffffffff0)))
      (aj (a c))
      ("compare ai aj" -1))
    ("bug in Flags.compare"
      (i 12)
      (j 6)
      (ai ((c) (unrecognized_bits 0xfffffffffffffff0)))
      (aj (b c))
      ("compare ai aj" -1))
    ("bug in Flags.compare"
      (i 12)
      (j 7)
      (ai ((c) (unrecognized_bits 0xfffffffffffffff0)))
      (aj (a b c))
      ("compare ai aj" -1))
    ("bug in Flags.compare"
      (i 12)
      (j 8)
      (ai ((c) (unrecognized_bits 0xfffffffffffffff0)))
      (aj ((a b c) (unrecognized_bits 0x3ffffffffffffff0)))
      ("compare ai aj" -1))
    ("bug in Flags.compare"
      (i 13)
      (j 2)
      (ai (
        (a                 c)
        (unrecognized_bits 0xfffffffffffffff0)))
      (aj (b))
      ("compare ai aj" -1))
    ("bug in Flags.compare"
      (i 13)
      (j 3)
      (ai (
        (a                 c)
        (unrecognized_bits 0xfffffffffffffff0)))
      (aj (a b))
      ("compare ai aj" -1))
    ("bug in Flags.compare"
      (i 13)
      (j 6)
      (ai (
        (a                 c)
        (unrecognized_bits 0xfffffffffffffff0)))
      (aj (b c))
      ("compare ai aj" -1))
    ("bug in Flags.compare"
      (i 13)
      (j 7)
      (ai (
        (a                 c)
        (unrecognized_bits 0xfffffffffffffff0)))
      (aj (a b c))
      ("compare ai aj" -1))
    ("bug in Flags.compare"
      (i 13)
      (j 8)
      (ai (
        (a                 c)
        (unrecognized_bits 0xfffffffffffffff0)))
      (aj ((a b c) (unrecognized_bits 0x3ffffffffffffff0)))
      ("compare ai aj" -1))
    ("bug in Flags.compare"
      (i 14)
      (j 1)
      (ai (
        (b                 c)
        (unrecognized_bits 0xfffffffffffffff0)))
      (aj (a))
      ("compare ai aj" -1))
    ("bug in Flags.compare"
      (i 14)
      (j 3)
      (ai (
        (b                 c)
        (unrecognized_bits 0xfffffffffffffff0)))
      (aj (a b))
      ("compare ai aj" -1))
    ("bug in Flags.compare"
      (i 14)
      (j 5)
      (ai (
        (b                 c)
        (unrecognized_bits 0xfffffffffffffff0)))
      (aj (a c))
      ("compare ai aj" -1))
    ("bug in Flags.compare"
      (i 14)
      (j 7)
      (ai (
        (b                 c)
        (unrecognized_bits 0xfffffffffffffff0)))
      (aj (a b c))
      ("compare ai aj" -1))
    ("bug in Flags.compare"
      (i 14)
      (j 8)
      (ai (
        (b                 c)
        (unrecognized_bits 0xfffffffffffffff0)))
      (aj ((a b c) (unrecognized_bits 0x3ffffffffffffff0)))
      ("compare ai aj" -1)) |}];
;;
