...
module To_and_of_string : sig
  ...
  module Date_and_ofday = struct
    ...
    let to_absolute relative ~offset_from_utc = sub (Span.of_int63_ns relative) offset_from_utc
    let of_absolute absolute ~offset_from_utc = Span.to_int63_ns (add absolute offset_from_utc)
    ;;
    ...
    | false ->
      ...
      let cache_start_incl =
        ...
        | true  -> effective_day_start
                   |> max (Zone.index_prev_clock_shift_time_exn zone index)
      in
      let cache_until_excl =
        ...
        | true  -> effective_day_until
                   |> min (Zone.index_next_clock_shift_time_exn zone index)
      in
  ...
  let to_ofday time ~zone =
    set_date_cache time ~zone;
    diff time date_cache.effective_day_start
    |> Ofday.of_span_since_start_of_day_exn
  ;;
