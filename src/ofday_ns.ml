...
let of_span_since_start_of_day_exn span =
  if is_invalid span
  then input_out_of_bounds span
  else span

let of_span_since_start_of_day_opt span =
  if is_invalid span
  then None
  else Some span
  ...
  module V1 = struct
    module T = struct
        ...
        fun t ~unit ->
          ...
          then "Incorrect day"
            ...
            let unit =
              ...
              | `Minute_or_less ->
                if ns <> 0 then `Nanosecond  else
                if us <> 0 then `Microsecond else
                if ms <> 0 then `Millisecond else
      ...
      let of_string string =
        Ofday_helpers.parse string ~f:create_from_parsed
...
let to_sec_string t =
  Stable.V1.to_string_with_unit t ~unit:`Second

let to_string_trimmed t =
  Stable.V1.to_string_with_unit t ~unit:`Minute_or_less
...
let%expect_test "small_diff" =
  ...
  in
  let examples =
    List.map ~f:(fun (x, y) -> (of_string x, of_string y)) [
      ("12:00", "12:05");
      ("12:58", "13:02");
      ("00:52", "23:19");
      ("00:00", "24:00");
  ...
  in
  List.iter examples ~f:(fun (x, y) -> test x y; test y x);
  [%expect {|
