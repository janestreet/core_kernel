...
module type Quickcheck_configured = sig
  ...
  (** [iter gen ~f] runs [f] on up to [trials] different values generated by [gen]. It
      ...
      It raises an exception if [f] raises an exception or if it fails to produce [trials]
      inputs from [gen] after [attempts] attempts. If [filter] is provided, iteration
      proceeds over [Generator.filter gen ~f:filter]. *)
  val iter
    :  ?seed     : seed
