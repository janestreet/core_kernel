...
module From_bytes =
  Test_blit.Make_distinct_and_test (Blit_elt) (Bytes_sequence)
    (struct
      external unsafe_blit
        :  src:bytes
        -> src_pos:int
...
module To_bytes =
  Test_blit.Make_distinct_and_test (Blit_elt) (Bigstring_sequence)
    (struct
      external unsafe_blit
        :  src:t
        -> src_pos:int
...
let uint64_to_int_exn n =
  if arch_sixtyfour
  then
    if n >= 0L && n < 0x4000_0000_0000_0000L
    then int64_to_int n
    else uint64_conv_error ()
  else if n >= 0L && n < 0x0000_0000_4000_0000L
  then int64_to_int n
  else uint64_conv_error ()
;;
...
let rec first_nonmatch ~buf ~pos ~max_pos ~char =
  ...
  then first_nonmatch ~buf ~pos:(succ pos) ~max_pos ~char
  else pos
;;
