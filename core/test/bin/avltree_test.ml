...
let test_f t s test_data f =
  let is_present i =
    ...
    match Avltree.find !t ~compare i with
    | None -> assert false
    | Some j -> assert (i = j)
  in
...
let do_add ~set ~t ~present ~i ~added =
  ...
  if present then assert (not !added) else assert !added;
  set := Set.add !set i;
  res
;;
...
let test_add t s test_data =
  ...
  test_f t s test_data (fun ~set ~t ~present ~i -> do_add ~set ~t ~present ~i ~added)
;;
...
let do_add_if_not_exists ~set ~t ~present ~i ~added =
  ...
  let res =
    Avltree.add
      ...
      ~added
      ~key:i
      ~data:(if present then i + 1 else i)
  in
  if present then assert (not !added) else assert !added;
...
let do_remove ~set ~t ~present ~i ~removed =
  ...
  if present then assert !removed else assert (not !removed);
...
let test_remove t s test_data =
  ...
  test_f t s test_data (fun ~set ~t ~present ~i -> do_remove ~set ~t ~present ~i ~removed)
;;
...
let test_add_remove t s test_data =
  ...
  test_f t s test_data (fun ~set ~t ~present ~i ->
    ...
    then (
      add := false;
      do_add ~set ~t ~present ~i ~added:r)
    else (
      add := true;
      do_remove ~set ~t ~present ~i ~removed:r))
;;
