(* INRIA's original command-line parsing library.
   The Command module is Core's preferred command-line parsing tool. *)


(* Note: the style of doc-comments and of the interface is not the standard in Core.
   That's because we've included INRIA's docs here, and we want to minimize the
   disruption..  *)

(* Includes material from INRIA.

   Copyright (c) 1996-2012 INRIA *)
