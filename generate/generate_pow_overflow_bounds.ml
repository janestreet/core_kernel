...
type type_info =
  { format_entry : Big_int.t -> string;
    ...
    (* [maybe_32bit=true] means that we should check at
       ...
       This only applies to Int31_native and Int63_native.
    *)
    maybe_32bit : bool
  }
...
let type_info_of_type =
  ...
  | Int63 ->
    { format_entry = (fun b -> Big_int.to_string b ^ "L");
      max_val = max_big_int_for_bits 63;
      name = "int63_on_int64";
      ocaml_type = "int64";
      generate_negative_bounds = false;
      maybe_32bit = false;
...
let print_array ~info ~descr arr =
  let name info = Printf.sprintf "%s_%s_overflow_bounds" info.name descr in
  Printf.printf
    "let %s : %s array =%s[|\n"
...
let gen_bounds ocaml_type =
  ...
  let name info = Printf.sprintf "overflow_bound_max_%s_value" info.name in
  Printf.printf
    "let %s : %s =%s%s\n\n"
    (name info)
    info.ocaml_type
...
let () =
  ...
  Printf.printf "#import \"config.h\"\n\n";
  gen_bounds Int32;
  Printf.printf "#ifdef JSC_ARCH_SIXTYFOUR\n\n";
  gen_bounds Int63_native;
  Printf.printf "#else\n\n";
  gen_bounds Int31_native;
  Printf.printf "#endif\n\n";
  gen_bounds Int63;
  gen_bounds Int64;
;;
